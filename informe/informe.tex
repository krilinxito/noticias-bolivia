...
El flujo de trabajo central es el siguiente: los artículos recién descargados se convierten en vectores numéricos mediante un modelo de lenguaje local (\textit{MiniLM-L12-v2}) y se agrupan en dos niveles jerárquicos. Primero se forman \textbf{episodios}: clusters estrictos (similitud $> 0{,}75$, sin repetir medio, ventana de 48 horas) que representan un hecho noticioso concreto --- un partido, una declaración, un decreto. Después, los episodios relacionados a lo largo del tiempo se agrupan en \textbf{temas} (similitud $> 0{,}65$, ventana de 14 días), que representan una historia en desarrollo. En ambos niveles, el artículo más cercano al centroide del cluster se selecciona como representante, garantizando que el título y la imagen del episodio o tema sean los más representativos semánticamente. Solo se crea un episodio si al menos \textbf{dos medios distintos} lo cubren --- esto garantiza que el sistema detecta únicamente noticias con cobertura cruzada real. Posteriormente, la API de Gemini analiza el tono editorial de cada artículo y detecta diferencias de encuadre entre los distintos medios que cubren el mismo episodio.
...
El \textbf{programador de tareas} (\textit{APScheduler}) coordina dos ciclos diarios: recolección a las 10:00 UTC y análisis a las 11:00 UTC. El ciclo de recolección descarga artículos nuevos y sus imágenes de portada; el ciclo de análisis agrupa los artículos en episodios y temas, y ejecuta el análisis editorial con Gemini. Ambos ciclos son independientes: el análisis siempre trabaja sobre los artículos ya almacenados en la base de datos.
...
La URL obtenida se almacena en la columna \texttt{imagen\_url} de la tabla \texttt{articulos}. El artículo representativo de cada episodio (seleccionado por centroide) provee su \texttt{imagen\_url} directamente al episodio y al tema. En el frontend, si la imagen no carga (error HTTP, URL expirada), el componente muestra una franja de color de la categoría del episodio como alternativa visual.
...
% ============================================================
\section{Agrupación en Episodios y Temas con IA Local}
% ============================================================
...
\begin{itemize}[nosep]
  ...
  \item $0.75$--$0.90$: artículos sobre el mismo hecho noticioso concreto.
  \item $0.65$--$0.75$: artículos sobre la misma historia con cobertura complementaria o en distintos momentos.
  \item $< 0.4$: artículos sobre temas no relacionados.
\end{itemize}

El sistema aplica \textbf{dos umbrales distintos} según el nivel de agrupación:

\begin{itemize}[nosep]
  \item \textbf{Episodios} (umbral $\mathbf{0.75}$): agrupa artículos publicados en las últimas \textbf{48 horas} que traten exactamente el mismo hecho --- una declaración, un partido, un decreto --- con la condición adicional de que sean de medios distintos y compartan al menos 2 palabras clave. El umbral alto garantiza que el cluster es un evento puntual, no un tema amplio.
  \item \textbf{Temas} (umbral $\mathbf{0.65}$): agrupa \textit{episodios} ya formados dentro de una ventana de \textbf{14 días}. Este nivel captura la historia en desarrollo: varios hechos puntuales relacionados con el mismo asunto (por ejemplo, distintas sesiones legislativas sobre el mismo proyecto de ley). El umbral más bajo permite capturar estos casos aunque cada episodio use vocabulario parcialmente diferente.
\end{itemize}

\subsection{Algoritmo Union-Find para clustering de episodios}

Con las similitudes calculadas, el problema se convierte en: \textit{¿cómo agrupar artículos en episodios?}. El algoritmo \textbf{Union-Find} (también llamado \textit{Disjoint Set Union}) resuelve esto eficientemente.

La idea es sencilla:
\begin{enumerate}[nosep]
  \item Al inicio, cada artículo está en su propio grupo individual.
  \item Por cada par de artículos $(i, j)$ que cumplan todas las condiciones: fusionar sus grupos.
  \item Al finalizar, todos los artículos en el mismo grupo conforman un episodio.
\end{enumerate}

Para la formación de episodios, la fusión de dos artículos solo ocurre si se cumplen \textit{todas} las condiciones siguientes:

\begin{itemize}[nosep]
  \item Similitud coseno $> 0{,}75$.
  \item Pertenecen a medios distintos (para garantizar cobertura cruzada desde el inicio).
  \item Comparten al menos 2 palabras clave extraídas por KeyBERT.
  \item Fueron publicados dentro de una ventana de \textbf{48 horas}.
\end{itemize}

\begin{lstlisting}[language=Python, caption=Pseudocódigo simplificado de Union-Find para episodios]
...
for i in articulos:
    for j in articulos:
        if (i != j
                and i.medio_id != j.medio_id
                and similitud(i, j) > 0.75
                and keywords_comunes(i, j) >= 2
                and diferencia_horas(i, j) <= 48):
            union(i, j)

# Resultado: grupos de articulos por find(id)
\end{lstlisting}

El resultado es un conjunto de \textit{clusters}: cada cluster contiene los artículos que el algoritmo considera que tratan el mismo hecho noticioso concreto. Las condiciones estrictas evitan los \textit{mega-clusters} en los que artículos de distintas semanas o distintos asuntos se agrupan por transitividad.

\subsection{Criterio de creación de episodio y tema}

\subsubsection*{Episodio}

No todo cluster se convierte en un episodio. Las condiciones de Union-Find ya exigen medios distintos por pares, pero el filtro final refuerza esto: \textbf{un cluster solo genera un episodio si contiene artículos de dos o más medios distintos}. Este requisito garantiza que el sistema solo almacena hechos con cobertura cruzada real, descartando artículos aislados.
...
nuevo_episodio = Episodio(
    titulo=articulo_centroide.titulo,
    imagen_url=articulo_centroide.imagen_url,
    score_importancia=len(medios_en_cluster) / 6
)
\end{lstlisting}

\subsubsection*{Tema}

Una vez formados los episodios del día, el sistema los agrupa en \textbf{temas} usando un segundo paso de Union-Find con el umbral laxo ($> 0{,}65$) sobre los embeddings centroides de cada episodio, dentro de una ventana de 14 días. El tema hereda el título e imagen del episodio más reciente y sirve como contenedor para rastrear una historia a lo largo del tiempo. Si un episodio no se agrupa con ningún otro, se le crea un tema propio (relación 1:1 inicial que puede crecer con el tiempo).

\subsection{Incorporación de artículos nuevos a episodios existentes}

Los hechos noticiosos no siempre terminan en un día. Una huelga puede generar nuevos artículos en días sucesivos. El sistema maneja este escenario comparando los clusters de artículos nuevos con los episodios ya existentes en la base de datos.

El proceso funciona así:

\begin{enumerate}[nosep]
  \item Se recuperan los episodios de los últimos \textbf{2 días} y se calcula el embedding centroide de cada uno.
  \item Para cada cluster de artículos nuevos, se calcula su \textbf{centroide} (promedio de embeddings).
  \item Se compara el centroide del cluster con el centroide de cada episodio existente.
  \item Si la similitud supera $0{,}75$ (umbral estricto): los artículos nuevos se incorporan al episodio existente en lugar de crear uno nuevo. Si no supera el umbral pero supera $0{,}65$, se crea un episodio nuevo que será agrupado en el mismo tema en el paso de temas.
\end{enumerate}

Esto evita que el mismo hecho aparezca como episodios duplicados y permite que el sistema acumule la cobertura de una noticia a lo largo de sus días de seguimiento.

\subsection{Selección del artículo representativo por centroide}

En cualquier cluster, el sistema debe elegir un artículo como \textbf{representante} para obtener su título e imagen. La elección influye directamente en lo que el usuario ve en las tarjetas y en el título del episodio o tema.

En lugar de elegir el primer artículo insertado (que puede ser el más periférico del cluster), el sistema calcula el \textbf{centroide} del cluster --- el vector promedio de todos los embeddings del grupo --- y selecciona el artículo cuyo embedding sea más cercano al centroide:

\begin{lstlisting}[language=Python, caption=Selección por centroide en embeddings.py]
import numpy as np

vectores = np.array([art.embedding for art in cluster])
centroide = vectores.mean(axis=0)

# Similitud de cada articulo con el centroide
sims = [np.dot(v, centroide) /
        (np.linalg.norm(v) * np.linalg.norm(centroide))
        for v in vectores]

representante = cluster[np.argmax(sims)]
titulo   = representante.titulo
imagen   = representante.imagen_url
\end{lstlisting}

Este artículo es el más ``típico'' del cluster en el sentido semántico: su embedding está más cerca del significado promedio del conjunto. Esto garantiza que el título y la imagen del episodio sean representativos del evento, no un caso borde o un artículo marginalmente relacionado.
...
Estas palabras clave se almacenan en el campo \texttt{keywords} del episodio y se usan para filtrado, categorización y presentación en el frontend.
...
Una vez que un artículo tiene cuerpo de texto \textbf{y pertenece a un episodio}, se envía a \textbf{gemini-2.5-flash-lite} para determinar su tono editorial. El requisito de pertenecer a un episodio es importante: no tiene sentido gastar llamadas a la API para analizar artículos que no van a participar en ninguna comparación de sesgos.
...
\midrule
Análisis de tono & \texttt{gemini-2.5-flash-lite} & \textasciitilde{}1500 req/día & Muchos artículos, tarea simple \\
\rowcolor{grisCelda}
...
\begin{itemize}[nosep]
  \item \textbf{Filtro por episodio}: solo se analizan artículos que ya pertenecen a un episodio (con \texttt{episodio\_id} asignado). Los artículos huérfanos no se envían a la API.
  ...
  \item \textbf{Pausa entre fases}: al terminar el análisis de tono se espera 65 segundos antes de iniciar el análisis de sesgo, para evitar que la fase de sesgo encuentre el límite por minuto recién saturado.
  \item \textbf{Reintento con backoff exponencial}: si la API devuelve un error 429 (\texttt{RESOURCE\_EXHAUSTED}), el sistema reintenta la llamada hasta 3 veces con una espera de $65 \times \text{intento}$ segundos entre reintentos (65s, 130s). Solo después del tercer fallo propaga el error.
  \item \textbf{Fallback de texto para sesgo}: si un artículo no tiene cuerpo de texto (\texttt{cuerpo} nulo), se usa el título como texto mínimo para el análisis de sesgo. Esto evita excluir al medio del análisis comparativo por ausencia de cuerpo, situación frecuente en portales como Red Uno que no siempre proveen el texto completo.
  \item \textbf{Degradación elegante}: si la cuota diaria se agota definitivamente, el ciclo registra el aviso y detiene el lote del día sin interrumpir el proceso completo.
\end{itemize}

Al limitar el análisis solo a artículos en episodios y ejecutar el ciclo una vez al día en lugar de cada hora, el sistema hace un uso mucho más eficiente de la cuota gratuita y evita los errores de rate limiting que afectaban al sistema anterior.
...
\rowcolor{grisCelda}
GET & \texttt{/temas} & \texttt{limit} & Temas agrupados por importancia \\
GET & \texttt{/temas/\{id\}} & --- & Detalle con lista de episodios \\
\rowcolor{grisCelda}
GET & \texttt{/episodios} & \texttt{limit} & Episodios con sesgo promedio \\
GET & \texttt{/episodios/\{id\}} & --- & Detalle con cobertura por medio \\
\rowcolor{grisCelda}
GET & \texttt{/sesgos} & \texttt{limit} & Episodios con mayor divergencia \\
...
El parámetro \texttt{orden} en \texttt{/noticias} acepta \texttt{fecha} (por fecha de publicación) o \texttt{scraping} (por fecha de recolección, útil para ver lo más reciente). El \texttt{score\_importancia} de un episodio se calcula como la fracción de medios monitoreados que lo cubren: $4/6 = 0{,}67$.

\subsection*{Ejemplo de respuesta de \texttt{/episodios/\{id\}}}

\begin{lstlisting}[caption=Respuesta abreviada del endpoint de detalle de episodio]
{
  "id": 123,
  ...
  "keywords": ["reforma tributaria", "impuesto", "gobierno"],
  "tema_id": 45,
  "articulos_por_medio": {
    "El Deber": [{
      "titulo": "...",
      ...
      "analisis": {"tono": "negativo", "sesgo": -0.3,
                   "sesgo_descripcion": "Encuadre critico hacia la medida"}
    }],
    "Los Tiempos": [{
      "titulo": "...",
      "imagen_url": null,
      "analisis": {"tono": "neutral", "sesgo": 0.1,
                   "resumen_comparativo": "El Deber enfatiza los criticos..."}
    }]
  }
}
\end{lstlisting}
...
\begin{itemize}[nosep]
  ...
  \item \texttt{get\_eventos}: obtiene los episodios más recientes o importantes.
...
\midrule
\texttt{episodio} & id, titulo, medios, score\_importancia, imagen\_url & Abre \texttt{/episodios/\{id\}} \\
\rowcolor{grisCelda}
...
La portada es la vista principal del sistema. Está implementada como un componente React interactivo (\texttt{PortadaContenido.tsx}) que recibe todos los temas del servidor y maneja el estado de filtrado en el cliente sin recargar la página.

\subsubsection*{Filtro de categorías}

En la parte superior de la portada aparece una fila de botones con las categorías detectadas automáticamente a partir de las palabras clave de los temas (Política, Economía, Deportes, Sociedad, Internacional, Cultura). Al hacer clic en una categoría, la vista se actualiza instantáneamente mostrando solo los temas de esa categoría. Un botón ``Todas'' restablece la vista completa.

Las categorías se infieren en el frontend a partir del campo \texttt{keywords} de cada tema, sin requerir ningún cambio en la base de datos: el módulo \texttt{categorias.ts} aplica un diccionario de palabras clave asociadas a cada categoría.

\subsubsection*{Tema destacado}

El primer tema de la categoría activa (o el más importante globalmente) se presenta en formato destacado: una tarjeta horizontal que ocupa todo el ancho disponible, con la imagen de portada a la izquierda y el texto a la derecha. Si la imagen no está disponible o no carga, la tarjeta ocupa el ancho completo con solo el texto.

El enlace de la tarjeta usa una lógica de \textbf{navegación directa}: si el tema tiene un único episodio, el clic lleva directamente a \texttt{/episodios/\{id\}} sin pasar por la página intermedia del tema. Si el tema tiene múltiples episodios, el clic lleva a \texttt{/temas/\{id\}} para mostrar la lista de episodios.

\subsubsection*{Grid de temas}

El resto de los temas se presenta en un grid de tres columnas adaptable al ancho de pantalla (\textit{responsive}). Cada tarjeta (\texttt{EventoCard}) muestra: imagen de portada (si existe), categoría con su color, título, barra de importancia y los medios que lo cubrieron. La misma lógica de navegación directa aplica a todas las tarjetas del grid.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Panel de estadísticas}: un botón ``$\equiv$ Stats'' en la cabecera del panel consulta el endpoint \texttt{GET /stats} y muestra un resumen del sistema: número de episodios y temas detectados, artículos recolectados, artículos analizados, medio más activo y episodio con mayor divergencia editorial. Este panel sirve como punto de entrada visual al estado actual del sistema.
\end{enumerate}

Cuando el asistente responde, el texto aparece acompañado de tarjetas (\textit{cards}) de episodios o artículos relacionados con la pregunta. Cada tarjeta es un enlace directo al episodio o al artículo original, con el título, el medio y (cuando está disponible) la imagen de portada.

\subsection{Temas (/temas) y Episodios (/episodios/[id])}

La página \texttt{/temas} es el listado completo de temas detectados, ordenados por fecha de detección y filtrables por categoría mediante parámetro de URL (\texttt{?categoria=Política}). Cada fila incluye el título del tema, los medios que lo cubren, el número de episodios y el tiempo relativo de detección.

La página \texttt{/temas/[id]} muestra los episodios de un tema con más de uno. Si el tema tiene un solo episodio (situación frecuente mientras el sistema acumula datos), la página realiza un \textbf{redirect 302} automático al episodio correspondiente, evitando el nivel intermedio.

La página \texttt{/episodios/[id]} es el detalle de un episodio concreto. Incluye:
\begin{itemize}[nosep]
  \item Cabecera con título, categoría, relevancia y medios cubriendo el episodio.
  \item Enlace de navegación \textit{ascendente} al tema padre (\texttt{↑ Ver tema}) cuando el episodio pertenece a un tema con múltiples episodios.
  \item \textbf{Sesgómetro} (\texttt{SesgometroComparativo.tsx}): eje horizontal $-1$ a $+1$ con un punto por cada medio. Solo aparece cuando hay análisis de sesgo completado para al menos 2 medios.
  \item \textbf{Cobertura por medio} (\texttt{CoberturaArticulos.tsx}): sección expandible por cada medio que cubre el episodio, con el artículo, su tono y enlace al original.
\end{itemize}

Las rutas antiguas \texttt{/eventos} y \texttt{/eventos/[id]} redirigen con código 301 a \texttt{/temas} y \texttt{/episodios/[id]} respectivamente, preservando la compatibilidad con enlaces externos ya publicados.
...
El elemento central es un gráfico SVG de tipo \textbf{beeswarm}: cada punto representa un episodio, su posición horizontal indica la fecha de detección, y su posición vertical se calcula automáticamente para evitar que los puntos se superpongan (de ahí el nombre ``beeswarm'' --- panal de abejas).

El gráfico codifica dos dimensiones de información:

\begin{itemize}[nosep]
  \item \textbf{Color}: representa el sesgo promedio del episodio entre sus medios.
    \begin{itemize}[nosep]
  ...
  \item \textbf{Tamaño}: proporcional al \texttt{score\_importancia} del episodio. Un episodio cubierto por los seis medios ($1.0$) tiene un radio máximo; uno cubierto por dos medios ($0.33$) tiene un radio mínimo.
\end{itemize}

Al pasar el cursor sobre un punto aparece un \textit{tooltip} con el título del episodio, la fecha, el sesgo promedio y el número de medios. Al hacer clic se navega al detalle del episodio (\texttt{/episodios/[id]}).
...
Umbrales coseno 0.75 (episodios) y 0.65 (temas)
  & Un solo umbral global
  & El umbral estricto (0.75) garantiza que los episodios son hechos puntuales concretos; el laxo (0.65) permite agrupar episodios de la misma historia aunque usen vocabulario parcialmente distinto. \\
\rowcolor{grisCelda}
...
\rowcolor{grisCelda}
2+ medios distintos para crear un episodio
  & Cualquier cluster
...
Chat responde con tarjetas estructuradas
  & Solo texto plano
  & Las tarjetas permiten navegación directa al episodio o artículo relevante, convirtiendo el chat en un punto de entrada a todo el sistema. \\
\rowcolor{grisCelda}
...
\rowcolor{grisCelda}
Dos niveles de clustering: episodios (sim $> 0{,}75$, 48h) y temas (sim $> 0{,}65$, 14 días)
  & Un solo nivel de eventos
  & Los episodios capturan el hecho puntual con criterios estrictos (distintos medios, keywords comunes, ventana corta). Los temas agrupan episodios relacionados a lo largo del tiempo, permitiendo rastrear historias en desarrollo. Un solo nivel con el umbral laxo generaba mega-clusters mezclando hechos no relacionados por transitividad. \\
Selección del artículo representativo por centroide semántico
  & Primer artículo insertado en el cluster
  & El centroide es el embedding promedio del cluster; el artículo más cercano a él es el que mejor representa el significado colectivo del grupo. El primer artículo puede ser periférico o el menos representativo si el cluster crece en días sucesivos. \\
\bottomrule
\end{tabular}
...
\subsection*{Episodio: partido de fútbol}

El episodio ``Bolívar empata con Nacional Potosí'' fue detectado con cobertura de cuatro medios (Red Uno, El Deber, Los Tiempos, Erbol), obteniendo un \textit{score} de importancia de $4/6 = 0{,}67$ (67\%). El sistema detectó este episodio porque cuatro redacciones distintas publicaron artículos similares el mismo día; al superar el requisito de 2+ medios y el umbral de similitud, se creó el episodio automáticamente. El artículo representante fue seleccionado por centroide.

\begin{table}[h]
\centering
\caption{Análisis de sesgo por medio para el episodio de fútbol}
\begin{tabular}{@{}lrl@{}}
\toprule
\rowcolor{azulAS!15}
...
En el gráfico de cronología, este episodio aparecería como un punto de tamaño mediano (67\% de importancia) con un color tirando a gris neutro (sesgo promedio cercano a $-0{,}17$), ubicado en la fecha de detección correspondiente.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Union-Find sin corrección dinámica}: una vez que un artículo se asigna a un episodio, no puede ser reasignado si el cluster crece con artículos que cambian su centro. La selección por centroide mitiga el problema del representante periférico, pero no reubica artículos ya asignados.

  \item \textbf{Baja cobertura cruzada}: la fragmentación regional de los medios bolivianos hace que muchos artículos solo sean cubiertos por uno o dos medios. El requisito de 2+ medios filtra correctamente estos casos, pero reduce el número total de episodios detectados. El análisis de sesgo comparativo requiere mínimo dos medios con texto (o título como fallback).

  \item \textbf{Jerarquía temas-episodios con datos escasos}: con pocos días de cobertura, cada episodio genera su propio tema en relación 1:1. La jerarquía de dos niveles se vuelve útil al acumular semanas de datos, cuando múltiples episodios de la misma historia comienzan a agruparse bajo un mismo tema.

  \item \textbf{Cronología de sesgo}: el gráfico beeswarm solo es informativo para episodios con análisis de sesgo ya completado. Episodios recientes aún en cola de análisis aparecen con color gris neutro, lo que puede confundirse con sesgo real de cero.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Resúmenes automáticos de episodios}: generar un resumen unificado que combine la perspectiva de todos los medios que cubrieron el mismo episodio, ofreciendo una visión sintética al lector.

  \item \textbf{Notificaciones push}: alertar a usuarios suscritos cuando se detecten episodios de alta importancia (score $> 0.8$) o divergencias editoriales elevadas (divergencia $> 0.7$).
