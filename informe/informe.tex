...
Este modelo más pequeño y económico es suficiente para un análisis por artículo individual. La petición incluye el título y los primeros 1500 caracteres del cuerpo, con temperatura 0.1 (valores bajos hacen la respuesta más determinista y reproducible). El \textit{prompt} solicita una respuesta en formato JSON:
...
El nivel gratuito de Gemini impone restricciones. Para no agotar la cuota, el ciclo de análisis aplica las siguientes estrategias:
...
En la esquina inferior derecha de toda la interfaz aparece un botón flotante con el texto ``$\star$ Analista IA''. Al hacer clic, se despliega un panel de chat de 22×30 cm que permite al usuario hacer preguntas en lenguaje natural sobre las noticias monitoreadas. El panel se puede cerrar en cualquier momento sin perder el historial de la conversación.

Cuando el asistente responde, el texto de la respuesta aparece acompañado de tarjetas de eventos o artículos relacionados con la pregunta. Cada tarjeta es un enlace directo al evento o al artículo original, con el título, el medio y (cuando está disponible) la imagen de portada.
