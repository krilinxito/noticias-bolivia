...
El flujo de trabajo central es el siguiente: los artículos recién descargados se convierten en vectores numéricos mediante un modelo de lenguaje local (\textit{MiniLM-L12-v2}) y se agrupan en \textbf{eventos}: clusters estrictos (similitud $> 0{,}75$, sin repetir medio, $\geq$2 palabras clave comunes, ventana de 48 horas) que representan un hecho noticioso concreto con cobertura cruzada real. El artículo más cercano al centroide del cluster se selecciona como representante, garantizando que el título y la imagen del evento sean los más representativos semánticamente. Solo se crea un evento si al menos \textbf{dos medios distintos} lo cubren; el \texttt{score\_importancia} (medios que cubren el evento entre 6) se recalcula automáticamente al final de cada ciclo. Posteriormente, la API de Gemini analiza el tono editorial de cada artículo y detecta diferencias de encuadre entre los distintos medios que cubren el mismo evento.

Los resultados son accesibles a través de una interfaz web con estética de periódico clásico: portada con tarjetas visuales (incluida la imagen de portada de cada evento, con fallback de franja de color cuando no hay imagen disponible), cronología de eventos con gráfico de dispersión por sesgo e importancia, y visualización detallada de sesgos editoriales. Adicionalmente, el sistema ofrece un asistente de inteligencia artificial conversacional flotante que responde preguntas sobre los datos monitoreados en lenguaje natural y devuelve tarjetas con los eventos o artículos más relevantes para cada consulta.
...
La URL obtenida se almacena en la columna \texttt{imagen\_url} de la tabla \texttt{articulos}. El artículo representativo de cada evento (seleccionado por centroide) provee su \texttt{imagen\_url} directamente al evento. En el frontend, si la imagen no carga (error HTTP, URL expirada), \texttt{EventoCard} muestra una franja de color de la categoría del evento como alternativa visual.
...
% ============================================================
\section{Agrupación en Eventos con IA Local}
% ============================================================
...
El sistema aplica un \textbf{umbral único de $0{,}75$} para la formación de eventos:

\begin{itemize}[nosep]
  \item \textbf{Union-Find} (umbral $\mathbf{0.75}$): agrupa artículos publicados en las últimas \textbf{48 horas} que traten exactamente el mismo hecho --- una declaración, un partido, un decreto --- con la condición adicional de que sean de medios distintos y compartan al menos 2 palabras clave. El umbral alto garantiza que el cluster es un hecho puntual y concreto.
  \item \textbf{Asignación a eventos existentes} (umbral $\mathbf{0.75}$): si el centroide del cluster nuevo supera ese umbral contra un evento de los últimos 3 días y comparte $\geq$2 palabras clave, los artículos se incorporan al evento existente en lugar de crear uno nuevo.
\end{itemize}

\subsection{Algoritmo Union-Find para clustering de eventos}
...
Para la formación de eventos, la fusión de dos artículos en el mismo cluster solo ocurre si se cumplen \textit{todas} las condiciones siguientes:
...
\begin{lstlisting}[language=Python, caption=Pseudocódigo simplificado de Union-Find para eventos]
...
El resultado es un conjunto de \textit{clusters}: cada cluster contiene los artículos que el algoritmo considera que tratan el mismo hecho noticioso concreto. Las condiciones estrictas evitan los \textit{mega-clusters} en los que artículos de distintos asuntos se agrupan por transitividad.

\subsection{Criterio de creación de evento}

No todo cluster se convierte en un evento. Las condiciones de Union-Find ya exigen medios distintos por pares, pero el filtro final refuerza esto: \textbf{un cluster solo genera un evento si contiene artículos de dos o más medios distintos}. Este requisito garantiza que el sistema solo almacena hechos con cobertura cruzada real, descartando artículos aislados.
...
Los hechos noticiosos no siempre terminan en un día. Una huelga puede generar nuevos artículos en días sucesivos. El sistema maneja este escenario comparando los clusters de artículos nuevos con los eventos ya existentes en la base de datos.

El proceso funciona así:

\begin{enumerate}[nosep]
  \item Se recuperan los eventos de los últimos \textbf{3 días} y se calcula el embedding centroide de cada uno.
  \item Para cada cluster de artículos nuevos, se calcula su \textbf{centroide} (promedio de embeddings).
  \item Se compara el centroide del cluster con el centroide de cada evento existente.
  \item Si la similitud supera $0{,}75$ \textbf{y} se comparten $\geq$2 palabras clave: los artículos nuevos se incorporan al evento existente y su \texttt{score\_importancia} se actualiza inmediatamente para reflejar los nuevos medios incorporados.
  \item Si no hay match, se crea un evento nuevo (siempre que el cluster cumpla el requisito de 2+ medios distintos).
\end{enumerate}

Al finalizar el ciclo completo, la función \texttt{recalcular\_scores\_eventos()} recorre todos los eventos activos y recalcula el score de importancia contando los medios distintos con artículos asociados, garantizando que los scores sean siempre consistentes con el estado real de la base de datos.

Esto evita que el mismo hecho aparezca como eventos duplicados y permite que el sistema acumule la cobertura de una noticia a lo largo de sus días de seguimiento.

\subsection{Selección del artículo representativo por centroide}

En cualquier cluster, el sistema debe elegir un artículo como \textbf{representante} para obtener su título e imagen. La elección influye directamente en lo que el usuario ve en las tarjetas y en el título del evento.

En lugar de elegir el primer artículo insertado (que puede ser el más periférico del cluster), el sistema calcula el \textbf{centroide} del cluster --- el vector promedio de todos los embeddings del grupo --- y selecciona el artículo cuyo embedding sea más cercano al centroide para representar el evento:
...
Este artículo es el más ``típico'' del cluster en el sentido semántico: su embedding está más cerca del significado promedio del conjunto. Esto garantiza que el título y la imagen del evento sean representativos del hecho noticioso, no un caso borde o un artículo marginalmente relacionado.
...
Estas palabras clave se almacenan en el campo \texttt{keywords} del evento y se usan para filtrado, categorización y presentación en el frontend.
...
\rowcolor{grisCelda}
GET & \texttt{/eventos} & \texttt{limit} & Eventos ordenados por fecha de detección \\
GET & \texttt{/eventos/\{id\}} & --- & Detalle con cobertura y análisis por medio \\
\rowcolor{grisCelda}
GET & \texttt{/sesgos} & \texttt{limit} & Eventos con mayor divergencia editorial \\
...
Las categorías se infieren en el frontend a partir del campo \texttt{keywords} de cada evento, sin requerir ningún cambio en la base de datos: el módulo \texttt{categorias.ts} aplica un diccionario de palabras clave asociadas a cada categoría.

\subsubsection*{Evento destacado}

El primer evento de la categoría activa (o el más importante globalmente) se presenta en formato destacado: una tarjeta horizontal que ocupa todo el ancho disponible, con la imagen de portada a la izquierda y el texto a la derecha. Si la imagen no está disponible, la tarjeta muestra una franja de color de la categoría con el título (el mismo fallback visual de \texttt{EventoCard}).

El enlace de cada tarjeta lleva directamente a \texttt{/eventos/\{id\}}.

\subsubsection*{Grid de eventos}

El resto de los eventos se presenta en un grid de tres columnas adaptable al ancho de pantalla (\textit{responsive}). Cada tarjeta (\texttt{EventoCard}) muestra: imagen de portada (si existe) o una franja de color de la categoría con el título del evento como \textbf{fallback visual}, categoría con su color, título, barra de importancia y los medios que lo cubrieron. El fallback se activa tanto cuando el artículo no tiene \texttt{imagen\_url} como cuando la imagen devuelve error HTTP al cargar.
...
\subsection{Eventos (/eventos y /eventos/[id])}

La página \texttt{/eventos} es el listado cronológico completo de todos los eventos detectados, filtrable por categoría mediante parámetro de URL (\texttt{?categoria=Política}). Cada fila incluye el título del evento, los medios que lo cubren y el tiempo relativo de detección.

La página \texttt{/eventos/[id]} es el detalle de un evento concreto. Incluye:
\begin{itemize}[nosep]
  \item Cabecera con título, categoría, relevancia y medios que cubren el evento.
  ...
  \item \textbf{Cobertura por medio} (\texttt{CoberturaArticulos.tsx}): sección expandible por cada medio que cubre el evento, con el artículo, su tono y enlace al original.
\end{itemize}

\subsection{Cronología (/cronologia)}

Esta vista reemplaza las páginas \texttt{/timeline} y \texttt{/serie} de versiones anteriores, unificándolas en una visualización más informativa y visualmente clara.
...
Al pasar el cursor sobre un punto aparece un \textit{tooltip} con el título del evento, la fecha, el sesgo promedio y el número de medios. Al hacer clic se navega al detalle del evento (\texttt{/eventos/[id]}).
...
Umbral coseno único 0.75 para clustering y matching
  & Umbral más bajo / sin umbral de matching
  & El umbral estricto (0.75) garantiza que los eventos son hechos puntuales concretos. Se aplica tanto al Union-Find (formación de clusters nuevos) como a la comparación con eventos existentes, evitando asignaciones erróneas por similitud superficial. \\
\rowcolor{grisCelda}
...
\rowcolor{grisCelda}
Fallback visual (franja de color) cuando no hay imagen
  & Imagen genérica / sin fallback
  & Cuando \texttt{imagen\_url} es nula o falla al cargar, \texttt{EventoCard} muestra una franja del color de la categoría del evento con el título. Esto mantiene la coherencia visual del grid sin depender de imágenes externas siempre disponibles. \\
...
3 modelos Gemini distintos por tarea (tono / sesgo / chat)
  ...
  & Cada modelo tiene su propia cuota gratuita diaria. Asignar tono al modelo de mayor cuota (\textasciitilde{}1500 req/día), sesgo al más capaz y chat a uno intermedio maximiza el uso sin coste adicional. Solo se analizan artículos con \texttt{evento\_id} asignado, evitando llamadas innecesarias para artículos sin cobertura cruzada. \\
...
Selección del artículo representativo por centroide semántico
  ...
  & El centroide es el embedding promedio del cluster; el artículo más cercano a él es el que mejor representa el significado colectivo del grupo. El primer artículo puede ser periférico o el menos representativo si el evento acumula artículos en días sucesivos. \\
\bottomrule
\end{tabular}
...
\begin{itemize}[nosep]
  \item \textbf{Disponibilidad de imágenes}: no todos los portales bolivianos publican el metadato \texttt{og:image} en sus artículos. Los eventos sin imagen (o cuyo artículo representante no tiene imagen) se muestran con la franja de color de la categoría como alternativa visual. Esto depende de la configuración interna de cada portal y escapa al control del sistema.
  ...
  \item \textbf{Union-Find sin corrección dinámica}: una vez que un artículo se asigna a un evento, no puede ser reasignado si el cluster crece con artículos que cambian su centro. La selección por centroide mitiga el problema del representante periférico, pero no reubica artículos ya asignados.

  \item \textbf{Baja cobertura cruzada}: la fragmentación regional de los medios bolivianos hace que muchos artículos solo sean cubiertos por uno o dos medios. El requisito de 2+ medios filtra correctamente estos casos, pero reduce el número total de eventos detectados. El análisis de sesgo comparativo requiere mínimo dos medios con texto (o título como fallback).
