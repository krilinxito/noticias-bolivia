...
El proceso se divide en dos etapas secuenciales por cada ciclo de 30 minutos:
...
\node[sbox, right=of t2] (t3) {Cuerpo + Fecha};
...
La \textbf{Etapa 1} (descubrimiento) descarga la portada de cada medio, extrae todos los hipervínculos y aplica un filtro por expresión regular para quedarse solo con URLs de artículos. La \textbf{Etapa 2} (extracción) visita cada URL nueva y extrae el cuerpo textual y la fecha de publicación.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Fallo silencioso}: si los 3 intentos fallan, retorna \texttt{None} y el artículo queda pendiente para el siguiente ciclo de 30 minutos.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Antigüedad máxima}: artículos con fecha de publicación superior a 30 días se descartan al inicio del sistema para evitar poblar la base de datos con contenido histórico irrelevante.
\end{itemize}
...
def correr_scraping(db):
    ...
    # Fase 2: extraer cuerpo de articulos sin completar
...
Esta función es invocada por \textbf{APScheduler} cada 30 minutos dentro del proceso FastAPI. La separación en dos fases es intencional: si un artículo nuevo se descubre en la Fase 1 pero falla la extracción de cuerpo (timeout, error HTTP), queda registrado en la BD con \texttt{cuerpo = NULL} y será reintentado en el siguiente ciclo de 30 minutos.
